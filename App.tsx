...
import { LayoutDashboard, Settings, Activity, Cloud, BarChart2, Menu, User, Lock, LogIn, LogOut, ShieldCheck, FolderInput, Wifi, X, Check, Cpu, Zap, Radio, Play } from 'lucide-react';
...
export default function App() {
  ...
  const handleSaveConfig = () => {
    // Save IP and disable simulation mode to force real connection attempt
    setEsp32Config(configIp, false);
    setIsSettingsOpen(false);
    // Reset data to show connection attempt visually
    setData(prev => ({ ...prev, online: false }));
  };

  // Toggle Camera Function (Global)
  const toggleGlobalCamera = async () => {
    // SECURITY CHECK: Warn user if they are on HTTP and not Localhost
    ...
    if (!isSecure && !isLocalhost) {
        alert(
          ...
          `Your browser has BLOCKED camera access because you are connecting via an insecure IP address (${window.location.hostname}).\n\n` +
          `Modern browsers only allow camera access on 'localhost' or 'https'.\n\n` +
          `HOW TO FIX:\n` +
          `1. Use 'localhost' if running on the same machine.\n` +
          `2. Or, on Chrome, go to 'chrome://flags/#unsafely-treat-insecure-origin-as-secure', enable it, and add this IP.\n`
        );
        return; 
    }
  ...
  // Helper: Save Snapshot to Local Drive or Selected Directory
  const captureAndSave = (sensorName: string) => {
    if (!hiddenVideoRef.current || !isCameraActive) return;

    const now = Date.now();
    // Cooldown: Only capture once every 5 seconds per type of event
    ...
    if (ctx) {
        ...
        // Add Timestamp & Reason text to image
        ctx.font = "24px monospace";
        ctx.fillStyle = "red";
        ctx.fillText(`ALERT: ${sensorName}`, 20, 40);
        ctx.fillText(`TIME: ${new Date().toLocaleString()}`, 20, 70);

        // Naming Convention: Image_Detected_[Sensor].png
        // Browser automatically handles .1, .2 if file exists in downloads
        const filename = `Image_Detected_${sensorName}.png`;

        canvas.toBlob(async (blob) => {
            if (!blob) return;

            // Method 1: Direct Save to Folder (if configured)
            if (saveDirHandle) {
                try {
                    ...
                    console.log(`Saved ${filename} directly to folder.`);
                    return; // Success, exit
                } catch (e) {
                    console.error("Direct save failed (permissions might be expired), falling back to download:", e);
                }
            }
  ...
  useEffect(() => {
    subscribeToESP32((newData) => {
      setData((prev) => {
          ...
          // 1. UPDATE HISTORY (Graph Data)
          // Append new point with ALL environmental metrics
          const newPoint = {
              time: timeString,
          ...
          // Keep only the last 30 points (increased slightly for better trend view)
          const updatedHistory = [...prev.history, newPoint].slice(-30);

          // 2. UPDATE LOGS (Triggers)
          ...
          if (newData.fenceActive !== undefined && newData.fenceActive !== null && newData.fenceActive !== prev.fenceActive) {
              if (prev.fenceActive !== null) { // Don't log initial connection state
          ...
          const updated = { 
              ...
              logs: newLogs.slice(0, 50) // Keep max 50 logs
          };
          
          // --- ALARM SNAPSHOT TRIGGERS ---
          if (isCameraActive) {
              ...
              if (updated.smoke && updated.smoke > 300) captureAndSave('Smoke');
  ...
  }, [isCameraActive, saveDirHandle]); // Re-bind if camera or storage handle changes
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    // Case-insensitive username check, case-sensitive password
    const user = AUTH_USERS.find(
      (u) => u.name.toLowerCase() === usernameInput.toLowerCase() && u.pass === passwordInput
    );

    if (user) {
      // START THE "DIVE" ANIMATION
      setIsDiving(true);
      
      // Wait for animation then log in
      setTimeout(() => {
          setCurrentUser(user);
          setLoginError('');
          setUsernameInput('');
          setPasswordInput('');
          setIsDiving(false);
          setIsPlayingIntro(true); // Start the Canvas Intro
      }, 1500);
    } else {
  ...
  // --- PLAYING INTRO VIDEO SCREEN (CANVAS) ---
  if (isPlayingIntro) {
      return <GenerativeIntro onComplete={finishLogin} />;
  }

  // --- MASS HERO CINEMATIC LOGIN ---
  if (!isAuthenticated) {
    return (
      <div 
      ...
      >
                    ...
                    <form onSubmit={handleLogin} className="space-y-6 relative z-10">
                        ...
                        <div className="space-y-1">
                            <div className="relative group">
                                ...
                                />
                                {/* Current Flow Animation on Focus */}
        ...
        {/* DIVING TRANSITION (White/Goldout) */}
        {isDiving && (
